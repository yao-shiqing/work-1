# reward--完整稿01
# reward 3 的设计（ra1-距离；ra2-同距离方向，rb-正确方向）
...
def scenario_3_reward(k, t, action_matrix, state_matrix,a_env,reward_matrix):
    ...
    zero_matrix_c = np.zeros((b1), dtype=int)
    for j in range (b1):
        if obs_k_t1[n + b2 * j] != 0:
            zero_matrix_c[j] = 1
    min_distance_c = float('inf')
    x_k0, y_k0 = state_matrix[t-1, k, 2], state_matrix[t-1, k, 3]
    for kc in range(b1):
        if zero_matrix_c[kc] == 1:
            k2 = kc if kc < k else kc + 1
            al_c_x, al_c_y = state_matrix[t-1, kc, 2], state_matrix[t-1, kc, 3]
            dis_c = np.sqrt((x_k0 - al_c_x)**2 + (y_k0 - al_c_y)**2)
            if dis_c < min_distance_c:
                min_distance_c = dis_c
                kc_n = kc 

    action_index_cn = next((idx for idx in range(2, 6) if action_matrix[t-1, kc_n, idx] == 1), None)
    cn = directions.get(action_index_cn, (0, 0))

    print('c_exp：',ce,'; c_t：',ct,)
    c1 = np.linalg.norm(np.array(ct) - np.array(ce))
    c2 = np.linalg.norm(np.array(ct) - np.array(cn))
    rb = (0.5 - c1)/3
    ra2 = (0.5 - c2)/3

    miu_a = 1- scenario_3_lamda(obs_k_t1, n, b1, b2)
    miu_b = scenario_3_lamda(obs_k_t1, n, b1, b2)
    r3 = miu_a * (0.5*ra1 + 0.5*ra2) + miu_b * rb
    ma = miu_a * (0.5*ra1 + 0.5*ra2) 
    mb = miu_b * rb
    print(f'scenario_3_reward: {r3:.4f}; ra1:{ra1:.4f}; ra2:{ra2:.4f}; rb:{rb:.4f}; miu_a*ra:{ma:.4f};  miu_b*rb:{mb:.4f}')
    reward_matrix[k,:] = r3 * action_matrix[t-1,k,:] + reward_matrix[k,:]*0.2
    return r3

'''
def tarcit(t,reward_matrix):
    nr = np.zeros((3, 1))
    nt = np.zeros((3, 1))
    tar = np.zeros((3, 1))
    for i in range(k+1):
        ...
        if reward[k] > 0:
            nr[lamda_id] += 1
            tar[lamda_id] = nr[lamda_id]/nt[lamda_id]
    for lamda_id in range(3):
        print(f'tar[{lamda_id}], {tar[lamda_id]}')
'''
