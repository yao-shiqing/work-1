# 初稿03 -- 增加 reward_matrix, 按照 reward 选择 action(并引入一定的随机性)
...
def main(map_name):
    ...
    for t in range(time-1):
        ...
        for agent in range(n_agents):
            ...
            if chose_action == 0:
                random_probs[chose_action] =random_probs[chose_action]
            else:
                random_probs[chose_action] = 1 + random_probs[chose_action]
        ...
        # 判断新局面
        k = 2  # 第(k+1)智能体
        estimate_scenario(k, a_env,t,state_matrix,action_matrix,reward_matrix)
        # print('time:',t,'g-state', state_matrix[t+1,:,:])
        fig_t(a_env,state_matrix[t+1,:,:],t+1)

def estimate_scenario(k, a_env,t,state_matrix,action_matrix,reward_matrix):
    ...
    if all(c_st_i == 0 for c_st_i in c_st):
        ...
        print(f"agent{k+1} 处于局面一","lamda_1:", lamda_1)
        ...
        print(f"agent{k+1} 处于局面三","lamda_3:", lamda_3)
        ...
        if state_all_zero:
            ...
            print(f"agent{k+1} 处于局面二", "lamda_2:", lamda_2)
...
# 这个地方的上限值比较低，就是移动速度（1/64）
    ...
    reward_matrix[k,:] = r1 * action_matrix[t-1,k,:] + reward_matrix[k,:]
    return r1*8

def scenario_2_reward(state_matrix,k,t,reward_matrix, action_matrix):
# 此场景下的上限值为 1/sight_ranges
    ...
    reward_matrix[k,:] = r2 * action_matrix[t-1,k,:] + reward_matrix[k,:]
    return r2
...
def scenario_3_reward(k, t, action_matrix, state_matrix,a_env,reward_matrix):
    ...
    reward_matrix[k,:] = r3 * action_matrix[t-1,k,:] + reward_matrix[k,:]
    return r3
